...
def hoja_calculo(request,data,nombre,inicio,fin):
        ...
        for i in data:
                ...
                ws.cell(row=cont,column=4).value = datetime.strftime(i['fechaInicio'],"%d/%m/%Y")
                ws.cell(row=cont,column=5).value = datetime.strftime(i['fechaFin'],"%d/%m/%Y")
                ws.cell(row=cont,column=6).value = i['idCliente__nombre']
                cont = cont + 1
