...
def reporte(request,datos,nombre,inicio,fin):
    ...
    data = [("Producto", "Cantidad Buscada", "Clientes")] # Este es el encabezado
    for i in datos:
        data.append((i['nombre'],'{}'.format(i['cantidad']), i['idCliente__nombre']))
    ...
    texto_pdf(c,h,w,"Productos más potenciales",12,80)
    texto_pdf(c,h,w,periodo,12,95)

    xlist = [x + x_offset for x in [50, 225, 350, 500]]
