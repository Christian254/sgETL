...
def reporte(request,datos,nombre,inicio,fin):
    ...
    data = [("Producto", "Inventario Inicial","Existencias", "Consigna")] # Este es el encabezado
    for i in datos:
        data.append((i['nombre'],'{}'.format(i['cantExistencia']),'{}'.format(i['disponible']),'{}'.format(i['consigna'])))
    ...
    texto_pdf(c,h,w,"Tardanza de movimiento de producto",12,80)
    texto_pdf(c,h,w,periodo,12,95)

    xlist = [x + x_offset for x in [0, 150,300, 400, 500]]
